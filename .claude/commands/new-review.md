...
1. **product-scraper** — obtén el producto:
   ...
   - Espera a que devuelva la ruta del JSON en `src/data/products/`.

2. **content-writer** — lee ese JSON y redacta la review MDX completa siguiendo las reglas de estilo. Guárdala en `src/content/reviews/<slug>.mdx`.

3. **seo-optimizer** — aplica la checklist SEO sobre el MDX recién creado.

4. **deploy-checker** — ejecuta las validaciones. Si todo pasa, informa al usuario con el resumen y la URL previsible (`https://{{DOMAIN}}/<slug>`).
